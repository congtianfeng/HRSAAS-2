...
router.beforeEach(async(to, from, next) => {
  ...
  if (store.getters.token) {
    ...
    if (to.path === '/login') next('/')// 有token还要去登录页强制跳转到主页
      ...
      // console.log(res)
      const data = await store.dispatch('permission/filterRoutes', res.roles.menus)
      router.addRoutes([...data, { path: '*', redirect: '/404', hidden: true }])
      // next(to.path)
      next({ ...to, replace: true })
    } else { next() }
  } else if (whiteList.includes(to.path)) { next() } else { next('/login') }
})
// 路由的拦截权限问题
// import router from '@/router'
// import store from '@/store'
